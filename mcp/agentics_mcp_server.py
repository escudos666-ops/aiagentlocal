...
def _service_catalog() -> dict[str, str]:
    return {
        ...
        "postgres": os.getenv("POSTGRAPHILE_HEALTH_URL", "http://postgraphile:5000/graphql"),
        "redis": os.getenv("REDIS_HTTP_HEALTH_URL", "http://redis:6379"),
        ...
        "browser_use_chrome_cdp": os.getenv("BROWSER_USE_CDP_HEALTH_URL", _browser_use_cdp_url()),
        "open_terminal": os.getenv("OPEN_TERMINAL_HEALTH_URL", "http://open-terminal:8000"),
        "waha": os.getenv("WAHA_HEALTH_URL", "http://waha:3000/api/sessions"),
        "postgraphile": os.getenv("POSTGRAPHILE_GRAPHQL_URL", "http://postgraphile:5000/graphql"),
...
def check_agentics_service(name: str) -> dict[str, Any]:
    ...
    return _request(services[name], timeout=HTTP_TIMEOUT)
...
def check_all_agentics_services() -> dict[str, Any]:
    ...
    return {
        name: _request(url, timeout=min(HTTP_TIMEOUT, 5))
        for name, url in _service_catalog().items()
    }
